...
class CustomCameraViewController: UIViewController, AVCapturePhotoCaptureDelegate {
    ...
    func openCamera(){
           ...
           case .notDetermined:
               ...
               if granted {
                   ...
                   Toast.show(message: "You need to allow camera access for this feature", controller: self)
                   }})
               break
           default:
       ...
       func setupCaptureSession(){
           ...
           do{
               ...
               print("Failed to set input with error: \(error)" )
           }
   ...
   @objc private func handleDismiss() {
    ...
    func photoOutput(_ output: AVCapturePhotoOutput, didFinishProcessingPhoto photo: AVCapturePhoto, error: Error?) {
        ...
        if let presenter = presentingViewController as? OnBoardTwoViewController {
            presenter.imageCaptured = previewImage
            presenter.avatarImageView.maskCircle(anyImage: previewImage!)
            presenter.avatarNotNil = true
        }
        if let presenter = presentingViewController as? EditProfileViewController {
            presenter.imageCaptured = previewImage
            presenter.profileImg.maskCircle(anyImage: previewImage!)
            presenter.avatarNotNil = true
        }
